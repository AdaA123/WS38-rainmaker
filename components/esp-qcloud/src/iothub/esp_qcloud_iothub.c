...
#define CONFIG_LIGHT_PROVISIONING_BLECONFIG
#include <string.h>
...
static esp_err_t esp_qcloud_iothub_config(esp_qcloud_mqtt_config_t *mqtt_cfg)
{
    ...
    switch (esp_qcloud_get_auth_mode()) {
        case QCLOUD_AUTH_MODE_KEY: {
            ...
            //  ESP_LOGW(TAG, "client_id: %s", mqtt_cfg->client_id);
            //  ESP_LOGW(TAG, "username: %s", mqtt_cfg->username);
            //  ESP_LOGW(TAG, "password: %s", mqtt_cfg->password);
            //  ESP_LOGW(TAG, "host: %s", mqtt_cfg->host);
...
esp_err_t esp_qcloud_iothub_init()
{
    ...
    return esp_event_post(QCLOUD_EVENT, QCLOUD_EVENT_IOTHUB_INIT, NULL, 0, portMAX_DELAY);
}

esp_err_t esp_qcloud_iothub_deinit()
{
    esp_err_t err = ESP_FAIL;
    if(g_iothub_group != NULL){
        vEventGroupDelete(g_iothub_group);
    }
    err = esp_qcloud_mqtt_deinit();
    ESP_QCLOUD_ERROR_CHECK(err != ESP_OK, err, "esp_qcloud_mqtt_init");

    return esp_event_post(QCLOUD_EVENT, QCLOUD_EVENT_IOTHUB_DEINIT, NULL, 0, portMAX_DELAY);
}
...
// #ifdef CONFIG_LIGHT_PROVISIONING_BLECONFIG
           /* This is an asynchronous operation, need to wait for the data to be sent */  
            esp_qcloud_prov_ble_report_bind_status(result_code);//綁定成功判定
// #endif
        if(0 == result_code){
            ESP_LOGI(TAG, "Successfully bind to the cloud");
            ESP_LOGI(TAG, "Waiting to be bound to the family");
        } else {
            ESP_LOGW(TAG, "QCLOUD_EVENT_IOTHUB_BIND_EXCEPTION");
...
esp_err_t esp_qcloud_iothub_bind(void)
{
    esp_err_t err = ESP_FAIL;
    
    cJSON *data_json = cJSON_CreateObject();
    cJSON_AddStringToObject(data_json, "token", esp_qcloud_prov_get_token());
    ...
    for(int i = 0; i < 5; ++i) {
        ...
        ESP_LOGW(TAG, "app_bind_token retry, count: %d", i + 1);
    }
...
esp_err_t esp_qcloud_iothub_connect(void)
{
    return esp_qcloud_mqtt_connect();
}

esp_err_t esp_qcloud_iothub_reconnect(void)
{
    return esp_qcloud_mqtt_reconnect();
}

esp_err_t esp_qcloud_iothub_disconnect(void)
{
    return esp_qcloud_mqtt_disconnect();
}

esp_err_t esp_qcloud_iothub_things_init(void)
{
    esp_err_t err = ESP_FAIL;
    ...
    return esp_event_post(QCLOUD_EVENT, QCLOUD_EVENT_IOTHUB_CLOUD_CONNECT, NULL, 0, portMAX_DELAY);
}

esp_err_t esp_qcloud_iothub_things_deinit()
{
    // TODO
    return ESP_FAIL;
}
