...
int app_rainmaker_init(void)
{
    ...
    */
	...
	// for (int i = SWITCH_CHANNEL_1; i< SWITCH_CHANNEL_MAX; i++){
        ...
        para_name = app_relay_get_name(SWITCH_CHANNEL_1);
        sprintf(esp_para, "esp.param.Power%d", SWITCH_CHANNEL_1 + 1);
        esp_rmaker_param_t* relay_status = esp_rmaker_param_create(para_name, esp_para,
            esp_rmaker_bool(app_relay_get_state(SWITCH_CHANNEL_1)), PROP_FLAG_READ | PROP_FLAG_WRITE);
